...
/* 6.6 port: the WMT core is one combined module (mtk_wcn); this is called
 * from wmt_dev.c's WMT_init (the single module_init) instead of being its own
 * module_init (which would collide). */
int wmt_detect_driver_init(void)
{
...
void wmt_detect_driver_exit(void)
{
...
/* module_init/exit moved to wmt_dev.c (single combined mtk_wcn module). */
