...
{
	/* show_stack() is arch-internal (not EXPORT_SYMBOL) in 6.6; this is a
	 * debug-only path. Log instead of dumping the stack. */
	pr_info("wcn-osal: thread_show_stack(%s) requested (no dump in 6.6 port)\n",
		pThread->threadName ? pThread->threadName : "?");
}
...
INT32 osal_thread_stop(P_OSAL_THREAD pThread)
{
	...
	if ((pThread) && (pThread->pThread)) {
		...
		/* Invalidate the handle: a second kthread_stop() on an
		 * already-exited kthread is a use-after-free hang on 6.6 —
		 * the kthread struct is freed in the task exit path
		 * (free_kthread_struct, fork.c) and kthread_stop() has no
		 * double-stop guard (kernel/kthread.c:696 waits
		 * kthread->exited unconditionally). wmt_lib_deinit() does
		 * stop + destroy on the same thread; the destroy must
		 * become a no-op. */
...
/* 6.6: timer callback now takes struct timer_list * (init_timer/function/data
 * removed). Trampoline recovers the OSAL_TIMER and calls the vendor handler. */
static void wcn_osal_timer_cb(struct timer_list *t)
{
	P_OSAL_TIMER pTimer = from_timer(pTimer, t, timer);
...
INT32 osal_timer_create(P_OSAL_TIMER pTimer)
{
	...
	timer_setup(timer, wcn_osal_timer_cb, 0);
	return 0;
}
...
INT32 osal_timer_start(P_OSAL_TIMER pTimer, UINT32 ms)
{
	...
	/* 6.6: add_timer() == mod_timer(timer, jiffies); schedule with the
	 * intended expiry via mod_timer() directly. */
	mod_timer(timer, jiffies + (ms / (1000 / HZ)));
	return 0;
}
