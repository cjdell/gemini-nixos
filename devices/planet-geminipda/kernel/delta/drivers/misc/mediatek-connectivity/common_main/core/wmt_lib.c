...
UINT32 wmt_lib_wait_event_checker(P_OSAL_THREAD pThread)
{
	...
	if (pThread) {
		...
		/* must include the stop check: the thread sits in
		 * wait_event_interruptible (no timeout, no signal check in
		 * the plain wait macro), so without it kthread_stop() only
		 * wakes it once, the condition is false on an empty opQ and
		 * the thread sleeps again forever (rmmod hang). Same pattern
		 * as _stp_psm_wait_for_msg/_stp_btm_wait_for_msg. */
