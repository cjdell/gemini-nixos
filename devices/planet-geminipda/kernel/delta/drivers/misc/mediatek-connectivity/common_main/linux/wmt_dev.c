...
/* One-shot diagnostic buffer: seeking not supported. */
static loff_t wmt_aee_proc_lseek(struct file *filp, loff_t offset, int whence)
{
	return -ESPIPE;
}
...
/* combo detect chardev init/exit (was its own module_init; now folded into the
 * single combined mtk_wcn module). */
extern int wmt_detect_driver_init(void);
extern void wmt_detect_driver_exit(void);

/* Gemini PDA glue: bring-up trigger on debugfs (wcn_hw_glue.c). */
extern int wcn_glue_dbgfs_init(void);
extern void wcn_glue_dbgfs_exit(void);
...
static INT32 WMT_init(VOID)
{
	...
	if (wmt_detect_driver_init() != 0)
		WMT_ERR_FUNC("wmt_detect_driver_init failed (detect chardev)\n");

	/* Gemini PDA (B-21): the vendor sets the chip type via
	 * do_connectivity_driver_init(chip_id) (the modular init path this
	 * combined module does not use); without it g_chip_type stays
	 * INVALID, wmt_lib_init() skips wmt_conf_read_file() (WMT_SOC.cfg)
	 * and wmt_core_stp_init() aborts on wmt_conf_get_cfg() == NULL.
	 * SoC chip id 0x6797 -> WMT_CHIP_TYPE_SOC (on-die CONSYS). */
	wmt_detect_set_chip_type(wmt_plat_get_soc_chipid());
	WMT_INFO_FUNC("chip type set from soc chipid 0x%x -> %d\n",
		wmt_plat_get_soc_chipid(), wmt_detect_get_chip_type());
...
#if WMT_CREATE_NODE_DYNAMIC
	/* 6.6: class_create() no longer takes the module owner. */
	...
	ret = wcn_glue_dbgfs_init();
	if (ret)
		WMT_WARN_FUNC("wcn glue dbgfs init failed (%d)\n", ret);
