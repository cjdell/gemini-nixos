		...
		if (gpWmtFuncOps[drvType] && gpWmtFuncOps[drvType]->func_on) {
			...
			if (WMTDRV_TYPE_WIFI == drvType) {
				if (WMT_HIF_UART == gMtkWmtCtx.wmtHifConf.hifType) {
					...
					/* [port 6.6, B-32] BTIF = the on-die CONSYS (MT6797
					 * 0x279, SOC-type): no SDIO slot to arm; func_on below
					 * drives the wlan probe cb / gWifiProbed. */
					if (WMT_HIF_BTIF != gMtkWmtCtx.wmtHifConf.hifType)
