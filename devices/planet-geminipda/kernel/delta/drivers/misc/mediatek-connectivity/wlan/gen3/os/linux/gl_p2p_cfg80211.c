...
int mtk_p2p_cfg80211_del_station(struct wiphy *wiphy, struct net_device *dev, struct station_del_parameters *params)
{
	...
	const u8 *mac;	/* [port 6.6] del_station now takes station_del_parameters */
	UINT_8 ucRoleIdx = 0;
 ...
 * update_mgmt_frame_registrations (whole-set callback); map the two filtered
 * subtypes onto the same packet-filter bits as the STA path. */
