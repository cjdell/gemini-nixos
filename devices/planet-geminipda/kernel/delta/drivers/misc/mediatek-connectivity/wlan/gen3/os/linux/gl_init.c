...
*/
#include "gl_os.h"
/* [port 6.6] struct sched_param for sched_setscheduler() now lives in
 * <uapi/linux/sched/types.h> (kernel/sched/core.c includes it explicitly). */
...
static const struct wiphy_vendor_command mtk_wlan_vendor_ops[] = {
	{
		...
		.policy = VENDOR_CMD_RAW_DATA, /* [port 6.6] 6.6 WARNs if policy is NULL; cmds take raw payloads */
...
VOID wlanSchedScanStoppedWorkQueue(struct work_struct *work)
{
	...
	/* [port 6.6] cfg80211_sched_scan_stopped() now takes (wiphy, reqid); the
	 * legacy (reqid==0) request is the one this driver starts. */
...
static int wlanStop(struct net_device *prDev)
{
	...
	if (prScanRequest)
		cfg80211_scan_done(prScanRequest, &(struct cfg80211_scan_info){.aborted = true});  /* [port 6.6] cfg80211_scan_done takes a scan_info now */
...
static void createWirelessDevice(void)
{
	...
	/* [port 6.6] WIPHY_FLAG_SUPPORTS_SCHED_SCAN no longer exists (removed
	 * upstream); sched-scan support is implied by the ops + max_sched_scan_*
	 * fields above. */
...
bailout:
		...
		if (prGlueInfo->prAdapter->rWifiVar.ucThreadPriority > 0) {
			/* [port 6.6] sched_setscheduler() is no longer exported (only the
			 * *_nocheck family): sched_setattr_nocheck() with a sched_attr is
			 * the direct equivalent for kernel-owned threads. */
			struct sched_attr attr = {
				.size = sizeof(struct sched_attr),
				.sched_policy = prGlueInfo->prAdapter->rWifiVar.ucThreadScheduling,
				.sched_priority = prGlueInfo->prAdapter->rWifiVar.ucThreadPriority,
			};

			sched_setattr_nocheck(prGlueInfo->main_thread, &attr);
#if CFG_SUPPORT_MULTITHREAD
			sched_setattr_nocheck(prGlueInfo->hif_thread, &attr);
			sched_setattr_nocheck(prGlueInfo->rx_thread, &attr);
#endif
			DBGLOG(INIT, INFO,
			       "Set pri = %d, sched = %d\n",
...
/* 20150205 work queue for sched_scan */
	...
	if (!wait_for_completion_timeout(&prGlueInfo->rHifHaltComp, MSEC_TO_JIFFIES(KAL_WLAN_REMOVE_TIMEOUT_MSEC))) {
		...
		sched_show_task(prGlueInfo->hif_thread);  /* [port 6.6] show_stack(task,NULL) -> sched_show_task */
	}
	...
	if (!wait_for_completion_timeout(&prGlueInfo->rRxHaltComp, MSEC_TO_JIFFIES(KAL_WLAN_REMOVE_TIMEOUT_MSEC))) {
		...
		sched_show_task(prGlueInfo->rx_thread);  /* [port 6.6] show_stack(task,NULL) -> sched_show_task */
	}
#endif
...
/* [port 6.6] MODULE_SUPPORTED_DEVICE() was removed upstream (empty in the
 * old module.h; the line was a no-op). */
MODULE_LICENSE("GPL");
