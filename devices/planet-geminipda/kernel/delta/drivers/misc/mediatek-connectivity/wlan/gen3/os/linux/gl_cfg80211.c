...
/* [port 6.6] cfg80211_ops.mgmt_frame_register was replaced by
 * update_mgmt_frame_registrations (kernel ~v5.x): the core now passes the
 * whole desired registration set (bitmap indexed by management-subtype) at
 * once instead of one frame type per call. Map the two types the vendor
 * firmware filters (PROBE_REQ / ACTION, subtypes 4 / 13) onto the same
 * packet-filter bits. */
void mtk_cfg80211_update_mgmt_frame_registrations(IN struct wiphy *wiphy,
						  IN struct wireless_dev *wdev,
						  IN struct mgmt_frame_regs *upd)
{
	u32 stypes = upd ? upd->interface_stypes : 0;

	mtk_cfg80211_mgmt_frame_register(wiphy, wdev, MAC_FRAME_PROBE_REQ,
					 !!(stypes & BIT(MAC_FRAME_PROBE_REQ)));
	mtk_cfg80211_mgmt_frame_register(wiphy, wdev, MAC_FRAME_ACTION,
					 !!(stypes & BIT(MAC_FRAME_ACTION)));
			      ...
			      IN struct net_device *ndev, IN struct cfg80211_sched_scan_request *request)
{
	...
	/* [port 6.6] request->interval is gone; intervals now live in the scan
	 * plans (scan_plans[0].interval), with request->delay as the legacy
	 * fallback. */
	prSchedScanRequest->u2ScanInterval = (UINT_16) ((request->n_scan_plans > 0) ?
				request->scan_plans[0].interval : request->delay);
...
int mtk_cfg80211_sched_scan_stop(IN struct wiphy *wiphy, IN struct net_device *ndev, IN u64 reqid)
{
	P_GLUE_INFO_T prGlueInfo = NULL;
	WLAN_STATUS rStatus;
	UINT_32 u4BufLen;
				...
				struct station_parameters *params)
{
	...
	if (params == NULL)
		return 0;
	/* [port 6.6] supported rates / HT / VHT capabilities moved into
	 * params->link_sta_params (the MLO link parameters); capability +
	 * ext_capab stayed on station_parameters. */
...
/* fgIsTDLSlinkEnable = 0; */
	...
	prGlueInfo = (P_GLUE_INFO_T) wiphy_priv(wiphy);
	ASSERT(prGlueInfo);
	/* [port 6.6] del_station now receives a station_del_parameters; a NULL
	 * mac means "remove all" - not supported here, bail like an empty
	 * lookup would. */
	if (!params || !params->mac) {
		DBGLOG(REQ, WARN, "del_station: no MAC given, skip\n");
		return 0;
	}
...
INT_32 mtk_cfg80211_process_str_cmd(P_GLUE_INFO_T prGlueInfo, PUINT_8 cmd, INT_32 len)
{
	...
	/* [port 6.6] strnicmp() was removed; use strncasecmp(). */
