...
/* [port 6.6] net/cfg80211.h no longer defines the IEEE80211_BAND_* aliases
 * (the enum ieee80211_band was dropped upstream; drivers use NL80211_BAND_*
 * directly). The old names had the same values, so alias them for the
 * vendored code. */
...
/* [port 6.6] sched_clock() lives in <linux/sched/clock.h> (the old
 * <linux/sched_clock.h> only holds the registration API). sched_show_task()
 * (the 6.6 replacement for show_stack(task, NULL) hang dumps) is declared in
 * <linux/sched/debug.h>. */
...
struct _GLUE_INFO_T {
	...
	/* [port 6.6] vendor P_TIMEOUT_HANDLER (void fn(unsigned long)) invoked by
	 * the glKalTimerFn trampoline - timer->data is gone. */
	void (*pfOidTimeoutHandler) (unsigned long);
