 ...
 */
#include "precomp.h"
/* [port 6.6] ktime_get_real_ts64() + rtc_time64_to_tm() (see below). */
...
VOID StatsEnvRxTime2Host(IN P_ADAPTER_T prAdapter, struct sk_buff *prSkb)
{
	...
	/* [port 6.6] do_gettimeofday()/struct timeval/rtc_time_to_tm() are gone;
	 * use the 64-bit timekeeping API (ktime_get_real_ts64 is what
	 * do_gettimeofday did) + rtc_time64_to_tm(). */
	...
	"IPID 0x%04x src %d dst %d UP %d,delay %u us,int2rx %lu us,IntTime %llu,%u/%u,leave at %02d:%02d:%02d.%06ld\n",
			...
			tm.tm_hour, tm.tm_min, tm.tm_sec, (long)(tval.tv_nsec / NSEC_PER_USEC));
		break;
	default:
		break;
	}
