...
/* [port 6.6] linux/ftrace_event.h is gone; the MET packet-trace marker
 * (event_trace_printk) lives in <linux/trace_events.h> under CONFIG_TRACING.
 * The whole MET profiling block is dead with CFG_MET_PACKET_TRACE_SUPPORT=0
 * (see gl_kal.c); keep this only for CONFIG_TRACING builds. */
...
/* [port 6.6] pr_debug() is compiled out (no DEBUG); the vendor DBGLOG level
 * mask (aucDebugModule, set via /proc/net/wlan/dbgLevel "idx:mask") already
 * gates verbosity, so surface at pr_info for bring-up visibility. */
