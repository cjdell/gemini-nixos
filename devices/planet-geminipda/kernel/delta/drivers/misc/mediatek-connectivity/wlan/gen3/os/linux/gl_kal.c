...
WLAN_STATUS kalFirmwareOpen(IN P_GLUE_INFO_T prGlueInfo)
{
	...
	/* [port 6.6] get_fs()/set_fs() removed; filp_open() in kernel context
	 * does not need an address-limit override. */
	/* Get FW name table */
...
WLAN_STATUS kalFirmwareLoad(IN P_GLUE_INFO_T prGlueInfo, OUT PVOID prBuf, IN UINT_32 u4Offset, OUT PUINT_32 pu4Size)
{
	...
	/* [port 6.6] 6.6 f_op has no ->read (only ->read_iter); kernel_read()
	 * handles both. The vendor's f_op->read==NULL check made every load
	 * fail with "Open FW done" but mapping NULL. */
	if ((filp == NULL) || IS_ERR(filp)) {
		goto error_read;
	} else {
		loff_t pos = u4Offset;	/* [port 6.6] kernel_read instead of f_op->read */
		*pu4Size = kernel_read(filp, prBuf, *pu4Size, &pos);
	}
...
WLAN_STATUS kalRxIndicateOnePkt(IN P_GLUE_INFO_T prGlueInfo, IN PVOID pvPkt)
{
	...
	/* [port 6.6] netif_rx_ni() was removed; netif_rx() is safe from
	 * process context too. */
	netif_rx(prSkb);
...
kalIndicateStatusAndComplete(IN P_GLUE_INFO_T prGlueInfo, IN WLAN_STATUS eStatus, IN PVOID pvBuf, IN UINT_32 u4BufLen)
{
	...
	case WLAN_STATUS_MEDIA_CONNECT:
		...
		if (prGlueInfo->fgIsRegistered == TRUE) {
			...
			if (eStatus == WLAN_STATUS_ROAM_OUT_FIND_BEST) {
				/* [port 6.6] cfg80211_roamed_bss() became cfg80211_roamed()
				 * with a cfg80211_roam_info struct (and no gfp - the skb
				 * is allocated in process context). */
				struct cfg80211_roam_info info = {
					/* [port 6.6] MLO: the bss lives in links[0];
					 * valid_links == 0 means legacy (link 0 only). */
					.links[0].bss = bss,
				...
				cfg80211_roamed(prGlueInfo->prDevHandler, &info, GFP_KERNEL);
			} else {
	...
	case WLAN_STATUS_MEDIA_DISCONNECT_LOCALLY:
		...
		if (prGlueInfo->fgIsRegistered == TRUE) {
			...
			/* CFG80211 Indication */
			/* [port 6.6] cfg80211_disconnected() dropped gfp and gained
			 * locally_generated + bssid. */
	...
	case WLAN_STATUS_SCAN_COMPLETE:
		...
		if (prScanRequest != NULL)
			cfg80211_scan_done(prScanRequest, &(struct cfg80211_scan_info){.aborted = false});  /* [port 6.6] cfg80211_scan_done takes a scan_info now */
		break;
...
/* [port 6.6] timer callback trampoline: 6.6 timer callbacks receive
 * struct timer_list * (timer->data was removed); from_timer() recovers the
 * GLUE_INFO_T and the vendor-style P_TIMEOUT_HANDLER (fn(unsigned long)) is
 * called with the glue pointer as its "data" argument. */
static void glKalTimerFn(struct timer_list *t)
{
	P_GLUE_INFO_T prGlueInfo = from_timer(prGlueInfo, t, tickfn);

	if (prGlueInfo && prGlueInfo->pfOidTimeoutHandler)
		prGlueInfo->pfOidTimeoutHandler((unsigned long)prGlueInfo);
}
...
VOID kalOsTimerInitialize(IN P_GLUE_INFO_T prGlueInfo, IN PVOID prTimerHandler)
{
	...
	prGlueInfo->pfOidTimeoutHandler = (void (*)(unsigned long))prTimerHandler;
	timer_setup(&(prGlueInfo->tickfn), glKalTimerFn, 0);
}
...
struct file *kalFileOpen(const char *path, int flags, int rights)
{
	...
	/* [port 6.6] no set_fs; filp_open is used directly. */
...
UINT_32 kalFileRead(struct file *file, unsigned long long offset, unsigned char *data, unsigned int size)
{
	...
	/* [port 6.6] kernel_read replaces the vfs_read + set_fs(KERNEL_DS) pair. */
...
UINT_32 kalFileWrite(struct file *file, unsigned long long offset, unsigned char *data, unsigned int size)
{
	...
	/* [port 6.6] kernel_write replaces the vfs_write + set_fs(KERNEL_DS) pair. */
...
VOID kalSchedScanResults(IN P_GLUE_INFO_T prGlueInfo)
{
	ASSERT(prGlueInfo);
	scanReportBss2Cfg80211(prGlueInfo->prAdapter, BSS_TYPE_INFRASTRUCTURE, NULL);
	/* [port 6.6] cfg80211_sched_scan_results() now takes a reqid; 0 for
	 * the legacy single sched-scan this driver runs. */
...
{
	/* [port 6.6] struct timespec/get_monotonic_boottime() -> 64-bit */
...
INT_32 kalHaltLock(UINT_32 waitMs)
{
	...
	if (waitMs) {
		...
		if (rHaltCtrl.fgHeldByKalIoctl) {
			...
			if (prGlueInfo)
				sched_show_task(prGlueInfo->main_thread);  /* [port 6.6] show_stack(task,NULL) -> sched_show_task */
		} else {
			DBGLOG(INIT, ERROR, "halt lock held by %s pid %d longer than %u ms!\n",
				rHaltCtrl.owner->comm, rHaltCtrl.owner->pid,
				kalGetTimeTick() - rHaltCtrl.u4HoldStart);
			sched_show_task(rHaltCtrl.owner);  /* [port 6.6] show_stack(task,NULL) -> sched_show_task */
		}
		return i4Ret;
	}
	down(&rHaltCtrl.lock);
