...
VOID kalP2PIndicateScanDone(IN P_GLUE_INFO_T prGlueInfo, IN UINT_8 ucRoleIndex, IN BOOLEAN fgIsAbort)
{
	...
	do {
		...
		if (prScanRequest != NULL) {
			...
			cfg80211_scan_done(prScanRequest, &(struct cfg80211_scan_info){.aborted = fgIsAbort});  /* [port 6.6] cfg80211_scan_done takes a scan_info now */
		}
				 ...
				 IN PUINT_8 pucRxIEBuf, IN UINT_16 u2RxIELen, IN UINT_16 u2StatusReason)
{
	...
	do {
		...
		if (prP2pConnInfo) {
			...
			/* [port 6.6] cfg80211_disconnected() gained a
			 * locally_generated arg before gfp. */
		      ...
		      IN UINT_8 ucRoleIndex, IN P_STA_RECORD_T prCliStaRec, IN BOOLEAN fgIsNew)
{
	...
	do {
		...
		if (fgIsNew) {
			...
			/* [port 6.6] NL80211_STA_INFO_ASSOC_REQ_IES was removed
			 * (the IE fields ride along with cfg80211_new_sta). */
