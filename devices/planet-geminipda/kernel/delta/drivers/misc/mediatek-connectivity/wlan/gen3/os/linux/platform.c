...
static int nvram_read(char *filename, char *buf, ssize_t len, int offset)
{
	...
	/* [port 6.6] get_fs()/set_fs(KERNEL_DS) were removed (5.10+); the
	 * kernel_read() helper does the same (kernel-space buffer) without
	 * playing with the address limit. */
	...
	if (retLen < 0) {
		DBGLOG(INIT, INFO, "[nvram_read] : kernel_read failed %d!!\n", retLen);
		retLen = -1;
	}
...
static int nvram_write(char *filename, char *buf, ssize_t len, int offset)
{
	...
	/* [port 6.6] see nvram_read: kernel_write() replaces the set_fs dance. */
	...
	if (retLen < 0) {
		DBGLOG(INIT, INFO, "[nvram_write] : kernel_write failed %d!!\n", retLen);
		retLen = -1;
	}
