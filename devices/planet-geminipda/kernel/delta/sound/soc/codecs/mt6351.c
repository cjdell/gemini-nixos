...
/*
 * Headphone Volume default/mirror (FIX 2026-09-06, kernel #328).
 *
 * ZCD_CON2 (the 'Headphone Volume' register, 0..18 inverted, dB
 * -10..+8) reads 0x0000 = volume 18 = MAX at power-up and after codec
 * power events. mt_hp_event()'s anti-pop save/restore used to snapshot
 * that register at PRE_PMU and ramp back to it at POST_PMU, so every
 * stream opened AND closed at MAX volume — with the Gemini's external
 * speaker amps enabled (SoC pads 243/244) that is a loud, potentially
 * damaging blast. The snapshot is gone; ana_gain[] is now a pure
 * software mirror of the user's setting (updated by mt6351_hp_vol_put),
 * seeded with the safe default below at probe so the FIRST stream after
 * boot is already at a sane level.
 */
#define MT6351_HP_VOLUME_MAX	0x12	/* kctl units: 18 */
#define MT6351_HP_VOLUME_DEFAULT 5	/* user-verified comfortable (-5 dB) */
#define MT6351_HP_VOLUME_RAW_DEFAULT \
	(MT6351_HP_VOLUME_MAX - MT6351_HP_VOLUME_DEFAULT)

static int mt6351_hp_vol_put(struct snd_kcontrol *kcontrol,
			     struct snd_ctl_elem_value *ucontrol)
{
	...
	if (ret < 0)
		return ret;
	/* mirror the user's level (inverted: kctl v -> raw reg 0x12-v) so
	 * mt_hp_event restores THIS, never the wiped hardware value */
	priv->ana_gain[AUDIO_ANALOG_VOLUME_HPOUTL] =
		MT6351_HP_VOLUME_MAX - (int)ucontrol->value.integer.value[0];
	priv->ana_gain[AUDIO_ANALOG_VOLUME_HPOUTR] =
		MT6351_HP_VOLUME_MAX - (int)ucontrol->value.integer.value[1];
	return ret;
}
...
static const struct snd_kcontrol_new mt6351_snd_controls[] = {
	/* dl pga gain */
	{
		...
		.put = mt6351_hp_vol_put,
		.private_value = (unsigned long)&(struct soc_mixer_control){
			.reg = MT6351_ZCD_CON2,
			.rreg = MT6351_ZCD_CON2,
			.shift = 0,
			.rshift = 7,
			.max = MT6351_HP_VOLUME_MAX,
			.invert = 1,
		},
		.tlv.p = playback_tlv,
	},
		       ...
		       int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		...
		/* [FIX 2026-09-06] do NOT snapshot the register into ana_gain[]
		 * here: ZCD_CON2 reads 0x0000 (= volume 18, MAX) at power-up and
		 * after power events, so the PRE_PMU snapshot made every stream
		 * open (and close) at MAX volume — loud/possibly damaging with
		 * the external speaker amps on. ana_gain[] is now the software
		 * mirror of the user's 'Headphone Volume' (mt6351_hp_vol_put),
		 * seeded with a safe default at probe (mt6351_codec_init_reg).
		 */
...
static int mt6351_codec_init_reg(struct snd_soc_component *cmpnt)
{
	...
	/* [FIX 2026-09-06] seed the HP-gain mirror with the safe default
	 * and write it now: the first stream after boot otherwise opens at
	 * ZCD_CON2's power-up value (0x0000 = volume 18 = MAX). The mirror
	 * is what mt_hp_event restores on every open/close.
	 */
	priv->ana_gain[AUDIO_ANALOG_VOLUME_HPOUTL] =
		MT6351_HP_VOLUME_RAW_DEFAULT;
	priv->ana_gain[AUDIO_ANALOG_VOLUME_HPOUTR] =
		MT6351_HP_VOLUME_RAW_DEFAULT;
	regmap_write(cmpnt->regmap, MT6351_ZCD_CON2,
		     (MT6351_HP_VOLUME_RAW_DEFAULT << 7) |
		     MT6351_HP_VOLUME_RAW_DEFAULT);
