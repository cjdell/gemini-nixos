...
/*
 * The ADDA analog output on this silicon (MT6351 codec via the AFE ADDA
 * path) is 16-bit-only in practice: mtk_dai_adda_hw_params never programs
 * a data-width register (rate only), and >16-bit streams come out garbled
 * — S24_LE is refused at hw_params and S32_LE plays a sine as a square
 * wave / white noise (verified on hardware 2026-09-07).  The vendor
 * (Android) HAL also used 16-bit.  Keep the whole card honest at S16.
 */
