	...
	for (i = 0; i < ARM64_NCAPS; i++) {
		...
		if (system_has_cap) {
			...
			if (cpu_has_cap && !cpucap_late_cpu_permitted(caps)) {
				/*
				 * Bring-up (2026-09-04): allow a late CPU that brings an
				 * EXTRA capability the boot CPUs lacked, instead of
				 * cpu_die_early(). Needed for the MT6797 A72 cluster
				 * (cpu8/9): the A53-only boot system has no Spectre-v2
				 * (cap 58) requirement, the late A72 does — refusing it
				 * kills the A72 during early boot. Booting it unmitigated
				 * is acceptable on this trusted bring-up device.
				 */
				pr_warn("CPU%d: late cap %d (%s) extra vs system — ALLOWED (bring-up)\n",
					smp_processor_id(), caps->capability, caps->desc);
				continue;
			}
		}
	}
