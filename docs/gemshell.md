...
plus `gemsettings`, a small settings client (Wi-Fi / Bluetooth / Audio).
...
| Piece | Where | Notes |
|---|---|---|
| `gemshell` (compositor) | `pkgs/gemshell/` (bin `gemshell`) | single process: wayland server + GL renderer + input + UI |
| `gemsettings` (settings app) | `pkgs/gemshell/` (bin `gemsettings`) | xdg_toplevel wl_shm client, CPU-drawn UI; Wi-Fi/Bluetooth/Audio |
| derivation | `pkgs/gemshell.nix` | rustPlatform, native aarch64, release profile |
| session module | `services/gemshell.nix` | `services.gemshellDesktop.enable` (co-installed, marker-selected) |
| session wiring | `services/desktop-select.nix`, `services/scripts/gemini-desktop-apply`, `pkgs/gemcli/src/session.rs` | mode `gemshell` |
...
- **Wayland** — `wayland-server` 0.31 (system/libwayland backend) +
  ...
  GPU-direct apps).
- **Clients** — any standard wl_shm xdg-shell client: gemsettings,
  GTK apps (via GTK's wayland backend), Qt (ozone-wayland), gemdemo,
...
- **Status bar** (top, 44 px): time (left); right side tap targets:
  gear (opens gemsettings), speaker, bluetooth, wifi, battery (% +
  charge bolt). Tapping speaker/wifi/bt/battery opens gemsettings on
  that section (`gemsettings --section wifi|bluetooth|audio`).
...
## Status sources (standard interfaces)

- **Battery** — sysfs `/sys/class/power_supply/bq25890-battery-*`
  (capacity is voltage-derived, no fuel gauge — same estimate UPower
  shows; `status` for the charge bolt). No fork: read directly.
- **Wi-Fi** — NetworkManager over the system bus (`busctl get-property
  org.freedesktop.NetworkManager … Connectivity` + ActiveConnections).
- **Bluetooth** — BlueZ over the system bus (`org.bluez` `/org/bluez/hci0
  .Powered`).
- **Volume** — `wpctl get-volume @DEFAULT_AUDIO_SINK@` (PipeWire, the
  system session at /run/gemwl-audio).
- **Brightness** — `/sys/class/backlight/*/brightness` (world-writable
  via the plumbing udev rule; the gemcli devmem path is the fallback if
  the sysfs node is absent).

Status refresh: a background thread polls every 2 s; the tray redraws
on change (dirty flag).

## gemsettings

An xdg_toplevel wl_shm client (its own wayland-client event loop; one
1080×2160 ARGB8888 buffer, redrawn on change, CPU-drawn: rects + the
same font atlas + icons). Screens:

- **Wi-Fi** — radio toggle, `nmcli dev wifi list` (SSID/signal/
  security), tap → password field (touch keyboard input) →
  `nmcli dev wifi connect`, Disconnect on the active one.
- **Bluetooth** — power toggle (`bluetoothctl power`), scan
  (`bluetoothctl scan on/off`), device list, tap → connect/disconnect,
  trust.
- **Audio** — sink list (`wpctl status`), tap = `wpctl set-default`,
  volume slider (`wpctl set-volume`), mute.

Launched by the compositor from the status-bar gear or a section tap
(`gemsettings --section <s>`); one instance (a second launch raises
the first — the compositor tracks it by app_id `gemsettings`).
...
- x86_64 fast loop: `bash bin/gemshell-host-check.sh` (cargo check in a
  nix shell: cargo/rustc/pkg-config + wayland/libxkbcommon/libglvnd).
- aarch64 (real): `sudo nix build --store local .#packages.aarch64-linux.gemshell`
  (or the full toplevel; long builds under `bin/run-job.sh`).
- Cargo.lock is committed (crate set pinned).
...
2. Status bar: time advances; battery % ≈ UPower's; wifi icon tracks
   the NM connection; tapping wifi/bt/sound opens gemsettings.
3. gemsettings: connect a known Wi-Fi (SSID+password), toggle BT,
   pick a sink, move the volume slider (audio out works).
...
7. Record: RSS (compositor + gemsettings), fps, boot time, anything
   that needed a fix here + in the session log.
