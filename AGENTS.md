...
| **DOSBox-X with the Gemini keyboard fix** (2026-09-11, build-level): DOSBox-X's SDL2 input is scancode/position based and ignores the host XKB layout, and Fn is level-3 only (no scancode) → wrong UK symbols and no F-keys. Wrapper forces `[dos] keyboardlayout=uk` and seeds a complete mapper binding `mod2` (RALT = Fn) + number row to F1..F10. **Mapper section must be `[SDL2]`, not `[sdl]`** (`SDL_STRING`). On-glass pending | `pkgs/dosbox-x-gemini.{nix,sh}`, `config/dosbox-x/mapper-dosbox-x.map`, `bin/gen-dosbox-x-mapper.sh`, **`docs/desktop-plumbing.md` §DOSBox-X** |
