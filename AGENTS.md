...
| **Power states — reboot/poweroff research (2026-09-10)**: why `systemctl reboot` limboes (arm64 `machine_restart` no-handler halt) and poweroff is refused; source-verified fix = WDT SWRST restart handler + MT6351 `RTC_BBPU`=0x4309 poweroff over the pwrap regmap; planned delta driver `drivers/power/reset/mt6797-power.c` + on-glass test plan | **`docs/power-states.md`** |
| Audio (PipeWire S16 path), Wi-Fi (CONSYS+USB), desktop (gemwl), **Phosh nested (phoc → phosh shell; DEFAULT desktop since 2026-09-09)**, **LXQt nested (labwc → lxqt-session; alternative, off by default since 2026-09-09)** | `services/audio.nix`, `services/wifi.nix`, `services/desktop.nix`, `services/lxqt.nix`, `services/phosh.nix` (+ `services/scripts/start-lxqt-nested`, `config/lxqt/`; + `services/scripts/{prepare-phosh-session,start-phosh-shell}`) |
| **Desktop plumbing — DE-agnostic system services (2026-09-10)**: battery/AC via **UPower** (kernel Battery supply `bq25890-battery-N` — no fuel gauge, capacity voltage-derived; upower policy Ignore because gemini-battery-guard owns poweroff), Wi-Fi via **NetworkManager** (`services.geminiWifi.useNetworkManager`, default true; wlan0 CONSYS + wlan1 dongle, usb0 unmanaged, home networks as NM profiles), backlight **udev chmod 0666** + `brightnessctl` (no logind session for the system-service desktops). Bluetooth already standard (bluez). Physical volume/media keys NOT wired (phoc/gsd limitation — follow-up). **gen62 deployed + NM/upower/backlight verified on glass 2026-09-10** (battery icon still needs the new boot.img). **Touch (2026-09-10): a real 10-point multitouch `wl_touch` device, no cursor** — gemwl forwards every finger (`wlr_seat_touch_notify_*`, `GEMWL_TOUCH_POINTER_EMU=1` restores the old pointer emulation); the nested compositor's wlroots wayland backend (phoc 0.19.3 / labwc 0.18.2) re-emits it, so phosh/LXQt apps get native touch + pinch (protocol chain verified in journals + synthetic-finger test; on-glass finger test pending) | **`docs/desktop-plumbing.md`** (§Touch), `services/plumbing.nix`, `services/wifi.nix`, `pkgs/gemwl/gemwl.c` |
