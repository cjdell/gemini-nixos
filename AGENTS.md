...
| **Desktop/session selector — GNOME + COSMIC + console** (2026-09-10, build-level; COSMIC on-glass pending): GNOME and COSMIC are co-installed GDM Wayland *sessions* (only one owns the KMS panel per boot); a persistent marker `/var/lib/gemini/desktop` resolved before GDM picks `gnome`/`cosmic`, or `console` skips GDM and stays on fbcon tty1. `gemcli session set gnome\|cosmic\|console [--apply\|--reboot]`; `--reboot` = clean switch. COSMIC 1.6.0 is aarch64-cache-verified at the flake pin; `displayManager.defaultSession` is deliberately unset (the selector owns AccountsService) | **`docs/desktop-selection.md`** + `services/desktop-select.nix` + `services/scripts/gemini-desktop-apply` + `pkgs/gemcli/src/session.rs` |
