...
| **gemdemo "GEMINI: EXODUS" cinematic spacesynth demo** (0.2.0 complete rewrite 2026-09-09: direct single-framebuffer renderer + rewritten spacesynth engine + A72 request; 0.1.0 "AETHER" was on glass 2026-09-08 windowed/fullscreen but was judged too slow/flawed/broken-music → rewritten; 0.2.0 aarch64 build green, ON-GLASS pending) | `pkgs/gemdemo.nix` + `pkgs/gemdemo/` + **`docs/gemdemo.md`** (0.2.0 architecture + 60 fps strategy; receipts kept: DSA→glGen, single-interleaved VAO rule, RUNPATH dlopen deps, wl_egl_window, RGBA readback; 0.1.0 was in gen31 systemPackages — NOT switched) |
