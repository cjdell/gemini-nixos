...
| **gemshell — the custom Rust Wayland compositor + desktop** (2026-09-11, **aarch64 build green** — `js6rq8h7v1xfg61q1fhyzg4sadqixshm-gemshell-0.1.0`; on-glass bring-up checklist owed): single-process compositor (wayland-server xdg-shell/wl_shm + EGL/GBM on `geminipda-drm` card0 → panfrost via Mesa kmsro, the same GL floor as GNOME) + `gemsettings` client (wl_shm, Wi-Fi/BT/Audio); evdev keyboard (xkb, `gemini` layout) + 10-finger touch, compositor-drawn shell UI (status bar / launcher / workspaces / gestures); boots via `services.gemshellDesktop` (system service, marker-gated — `gemcli session set gemshell --reboot`). **Build gotchas:** libglvnd's libEGL exports no `eglGetPlatformDisplayEXT`/`eglCreatePlatformSurface` (use `eglGetProcAddress`); the store xkbcommon 1.13.1 (aarch64) exports only the V_0.5.0-era state API (modmap masks via `xkb_state_mod_index_is_active`); `c_char` = `u8` on aarch64; link set in `pkgs/gemshell/build.rs`. Host loop: `bash bin/gemshell-host-check.sh` (cargo check, seconds) | **`docs/gemshell.md`** + `pkgs/gemshell/` + `pkgs/gemshell.nix` + `services/gemshell.nix` |
