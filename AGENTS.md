...
| **gemdemo "AETHER" demoscene + GPU stress test** (2026-09-08 ON GLASS: windowed in labwc + fullscreen on gemwl, first sustained T880 load) | `pkgs/gemdemo.nix` + `pkgs/gemdemo/` + **`docs/gemdemo.md`** (receipt: DSA→glGen, single-interleaved VAO rule, RUNPATH dlopen deps, wl_egl_window, post-FBO-1×1, RGBA readback; in gen31 systemPackages — NOT switched) |
