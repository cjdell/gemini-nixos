...
| **gemdemo "GEMINI: EXODUS" cinematic spacesynth demo** (0.2.0 complete rewrite 2026-09-09: direct single-framebuffer renderer + rewritten spacesynth engine + A72 request; 0.1.0 "AETHER" was on glass 2026-09-08 windowed/fullscreen but was judged too slow/flawed/broken-music → rewritten; 0.2.0 ON GLASS windowed 2026-09-09: ~60 fps across the show incl. the S5 reveal after the baked-planet-map fix; audio live via gemini16; PPM dumps on the device await human eyes) | `pkgs/gemdemo.nix` + `pkgs/gemdemo/` + **`docs/gemdemo.md`** (0.2.0 architecture + 60 fps strategy; receipts kept: DSA→glGen, single-interleaved VAO rule, RUNPATH dlopen deps, wl_egl_window, RGBA readback; 0.1.0 was in gen31 systemPackages — NOT switched) |
