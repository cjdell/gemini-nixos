...
| Audio (PipeWire S16 path), Wi-Fi (CONSYS+USB), desktop (gemwl), **LXQt nested (labwc → lxqt-session)**, **Phosh nested (phoc → phosh shell; LXQt alternative, off by default)** | `services/audio.nix`, `services/wifi.nix`, `services/desktop.nix`, `services/lxqt.nix`, `services/phosh.nix` (+ `services/scripts/start-lxqt-nested`, `config/lxqt/`; + `services/scripts/{prepare-phosh-session,start-phosh-shell}`) |
