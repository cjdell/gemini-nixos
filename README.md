...
| `pkgs/gemdemo.nix` + `pkgs/gemdemo/` | **gemdemo — "GEMINI: EXODUS" (0.2.0, deployed as gen33 2026-09-09)**: cinematic spacesynth assembly for the Gemini — direct single-framebuffer renderer (sky/sprites/planet/ring/ship layers — no multipass/SSAA/raymarch; planet = baked startup maps + UV-scroll spin for the 60 fps reveal), full 126 BPM spacesynth score engine, A72-cluster request. On glass windowed 2026-09-09: ~60 fps every chapter, audio live via gemini16; 0.1.0 "AETHER" (gen32 and earlier) was the first sustained T880 load but was too slow/flawed/broken-music → rewritten. See docs/gemdemo.md. Run from the desktop: `gemdemo` (system path; add `--windowed` for a dev window, keys: space pause · [ ] chapter · h HUD · q quit) |
