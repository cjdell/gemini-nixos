...
| `pkgs/gemdemo.nix` + `pkgs/gemdemo/` | **gemdemo — "GEMINI: EXODUS" (0.2.0 rewrite, 2026-09-09, aarch64 build green — on-glass pending)**: cinematic spacesynth assembly for the Gemini — direct single-framebuffer renderer (sky/sprites/planet/ring/ship layers — no multipass/SSAA/raymarch), full 126 BPM spacesynth score engine, A72-cluster request; 0.1.0 "AETHER" was the first sustained T880 load on glass (2026-09-08) but was too slow/flawed/broken-music → rewritten. See docs/gemdemo.md (0.2.0 architecture + the kept fork receipts: DSA→glGen stubs, single-interleaved VAO rule, RUNPATH dlopen deps). Run from the desktop: `gemdemo --windowed` or fullscreen on gemwl (0.1.0 was in gen31 `systemPackages`; NOT switched yet) |
