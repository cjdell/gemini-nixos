...
| `pkgs/gemdemo.nix` + `pkgs/gemdemo/` | **gemdemo — "AETHER" demoscene + GPU stress test (2026-09-08, ON GLASS)**: Rust/EGL demoscene (raymarch, tunnel, stars, bloom/feedback post, fully synthesized audio) built native-aarch64 in-repo and verified live on the LXQt/labwc session + gemwl — the first real sustained load for the T880 panfrost fork. See docs/gemdemo.md (receipt: every blocker green-build → on-glass, incl. DSA→glGen stubs, single-interleaved VAO rule for the fork, RUNPATH dlopen deps). Run: `gemdemo --windowed --ssaa 1 --silent` from the desktop (in gen31 `systemPackages`; NOT switched yet) |
